...
Das vorliegende Kapitel stellt Technologie und Konzepte vor, welche den Stand der Technik und Forschung im Bereich der PLE-Entwicklung darstellen oder notwendig sind, um die in Kapitel \ref{chapter:Kapitel3} entwickelnden Anforderungen an eine webbasierte und offlinefähige Personal Learning Environment zu erfüllen. Das Kapitel wird mit einem Blick auf zwei unterschiedliche Konzepte zum Aufbau und zur Klassifikation von PLEs, welchem bei dem Design und der Evaluierung des Systems von Nutzen sind, eröffnet. Anschließend werden die aktuell verbreitetsten Widget-Frameworks beschrieben. Es folgen zwei Abschnitte zu dem technologischen Fundament, des Systems. Das Kapitel schließt mit einem Abschnitt über die Verwendung des Prototypen auf einem USB-Stick und einem kurzen Überblick über ähnliche auf dem Markt befindliche Systeme. 

\section{Konzepte zum Aufbau und zur Klassifikation von Personal Learning Environments}
TODO Einleitung, Bezug zur Arbeit
Das Z
Warum ist das wichtig, es soll eine PLE gebaut werden, wie soll diese aussehen. Kann benutzt werden um Anforderungen zu validieren(?!?) 
Im folgenden, Design Patterns helfen bei Entwicklung, Schablonen für den  Entwurf. 
Bezug zu Anforderungsanalyse.
...
\subsection{Einordnung der prototypischen Personal Learning Environment}
Diese Arbeit konzentriert sich vorrangig, auf die Umsetzung der Screen-Dimension und der Data-Dimension nach Palmér. Für einen funktionsfähigen Prototypen ist es notwendig, dass ein User-Interface des PLE-Containers erstellt und als in einem Browser lauffähig umgesetzt wird. Zur Umsetzung des User-Interfaces eignen sich inbsbesondere die von Wilson definierten "`Discourse Monitor"', "`Navigation Layer"' und  "`Choose Change and Discard"' Muster. Als Einstiegspunkt in das System kann ein "`Discourse Monitor"' dienen. Dieser fungiert als eine Startseite welche dem Nutzer die wichtigsten Informationen bezüglich seiner Widgets zur Verfügung stellt und ihm durch Links die Möglichkeit gibt direkt zu den gewünschten Personal-Learning-Tools zu gelangen. Der "`Navigation Layer"' muss in einer PLE, welche als Mashup-Anwendung konzipiert wurde wird inhärent mit eingebaut werden. Durch die Einbindung externer Widgets, wird dem Anwender nur der Funktionsumfang zur Verfügung gestellt, welche von den Widget-Entwicklern angedacht wurde. Für den vollen Funktionsumfang muss der Nutzer zum eigentlichen Service des Widgets navigieren. Die Widgets sollten dann innerhalb des Systems aggregiert und ihm in übersichtlicher Form in Form eines Dashboards präsentiert werden. Der Nutzer sollte in der Lage sein Widgets zu seinem Dashboard hinzuzufügen, zu entfernen oder über Drag and Drop nach seinen Wünschen anzuordnen. Dadurch wird auch das "`Choose Change and Discard"'-Pattern (siehe Seite \pageref{wilson_patterns:choose_change_discard}) grundsätzlich im System verankert.  

Eine der funktionalen Anforderungen an das System ist die Fähigkeit die wichtigsten Funktionalitäten auch offline weiterhin nutzen zu können und die anschließende Synchronisierung der Daten, wenn die Konnektivität wieder hergestellt ist. Diese Funktionalitäten beschäftigen sich also primär mit den Umgang mit Daten und Informationen und den Austausch ebendieser zwischen unterschiedlichen Systemen (zwischen dem Prototypen und den Widgets) und der Synchronisierung der Daten bei Statusänderung (von Offline- zu Online-Modus) und fallen somit in die Data-Dimension. Die Fähigkeit des Systems zu Erkennen, ob es zu einem Zeitpunkt online oder offline is kann als eine Erweiterung des von Wilson vorgestellten Multimode-Patterns betrachtet werden.   
Die anderen Dimensionen und Muster werden im im Zuge dieser Arbeit nicht implementiert. Am Ende wird also keine vollständige Personal Learning Environment, sondern eine System welches eine Teilmenge der erforderlichen Funktionen erfüllt als Ergebnis stehen.

\section{Widget-Frameworks}
TODO Einleitung/Bezug zu Arbeit
wie in wilson bla beschrieben, soll die PLE als Aggregator, Dashboard. Entwickelt werden. Auf Grund dessen werden die zur Verfügung gestellten Services mit Hilfe von Widgets (siehe Kapitel \ref{section:widgets}) realisiert. Im Folgenden werden zwei unterschiedliche Widget-Frameworks vorgestellt und es wird die Wahl eines der beiden zur Implementierung in der Arbeit begründet.
...
Nach der OpenSocial-Specification\cite{Opensocial2013} ist OpenSocial
\begin{quotation}
...
TODO Bezug, Zusammenfassung, Kritik
...
\section{Protokolle (TODO Schlauer Titel)}

TODO Einleitung/Bezug zu Arbeit
Bei der Erstellung von Mashup-Anwendungen, besondere Herausforderungen lalblal
...
Ein einfacher CORS-Request vom Client zum Server sieht wie folgt aus (Workflow analog zu \cite{Hossain2012}):
...
TODO Bezug, Zusammenfassung, Kritik, eventuell kürzen
...
TODO: Satz wie, das das benötigt wird. lalala
...
\section{Plesynd auf dem USB-Stick}
