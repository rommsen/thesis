...
\section{Evaluation}
In diesem Abschnitt wird die in Kapitel \ref{chapter:Kapitel5} vorgestellte Lösung analysiert

nichtfunktional: Szenario, fiktiver Walkthrough, keine Zeit für Usability tests etc

\subsection{Validierung der funktionalen Anforderungen}
Die Validierung der funktionalen Anforderungen befasst sich primär mit der Analyse der Use-Cases und deren Umsetzung in Plesynd. Hierzu wird Bezug auf die in Kaptitel \ref{chapter:Kapitel5} vorgestellten Komponenten und das User-Interface genommen, um die Erfüllung der Anforderungen zu validieren

\textbullet{} \emph{fA1: Der Anwender muss sich selbständig in dem registrieren können.}\\
Ist der Anwender noch nicht registriert, hat er die Möglichkeit ein Anmeldeformular (siehe Abbildung TODO) auszufüllen und dieses an den Server zu senden. Die gesendeten Daten werden validiert und führen Fehlerfall zu einer erneuten Darstellung des Formulars samt Fehlermeldung und im Erfolgsfall zu einer Mitteilung an den Anwender einem neuen Nutzer innerhalb des Plesynd-Systems.

\textbullet{} \emph{fA2: Der Anwender muss sich sich in das System mit Nutzername und Passwort einloggen können.}\\
Plesynd-Server hinterlegt die registrierten Anwender in einer Datenbank. Plesynd-Client bietet dem Nutzer die Möglichkeit sich über ein Formular an dem System anzumelden (siehe Abbildung TODO)

\textbullet{} \emph{fA3: Dem Anwender darf nur Zugriff auf die zu seinem Account gehörigen Workspaces und Widgets gewährt werden.}\\
Plesyndbenutzt wie Abbildung \ref{fig:ueberblick_plesynd_komponenten} zeigt auf Serverseite Access-Control-Lists und hinterlegt damit für jeden Workspace und jedes Widget welcher Anwender diese erstellt und damit auch Zugriff auf sie hat. Andere Nutzer erhalten auch, wenn sie den Link zu einem Workspace kennen keinen Zugriff darauf. Ähnliches gilt auch für die mit Hilfe des Todo-Listen-Widgets erstellten 

\textbullet{} \emph{fA4: Der Anwender muss sich aus dem System ausloggen können.}\\
Klickt der Anwender auf den Link Logout (siehe Abbildung TODO), beendet das System seine Session und leitet ihn wieder auf die Login-Seite um. 

\textbullet{} \emph{fA5: Nach Beendigung der Anwender-Session, darf kein Zugriff mehr auf die Daten des Anwenders bestehen.}\\
...
\textbullet{} \emph{fA6: Der Anwender muss neue Workspaces in seine Lernumgebung einfügen können.}\\
Mit einem Klick auf den Hinzufügen Button in der Reiternavigation des Systems wird direkt eine neuer Workspace zu dem System hinzugefügt.

\textbullet{} \emph{fA7: Der Anwender muss den Namen des Workspaces ändern können.}\\
...
\textbullet{} \emph{fA8: Der Anwender muss einen Workspace löschen können.}\\
...
\textbullet{} \emph{fA9: Der Anwender muss über ein Auswahlfeld Widgets zu Workspaces hinzufügen können.}\\
...
\textbullet{} \emph{fA10: Der Anwender muss die zur Auswahl stehenden Widgets über ein Suchfeld einschränken können.}\\
...
\textbullet{} \emph{fA11: Der Anwender muss die zur Auswahl stehenden Widgets so einschränken können, dass in der Liste nur offline-fähige Widgets vorkommen.}\\
Der in der Analyse der Anforderung \emph{fA10} vorgestellte Suchbereich, enthält neben dem Suchfeld noch einen Filter-Button, der es erlaubt nur offline-fähige Widgets anzuzeigen (siehe Abbildung \ref{fig:plesynd_workspace_edit}). Diese Filterung findet wie die Suche nur auf dem Client statt 

\textbullet{} \emph{fA12 Der Anwender muss ein Widget löschen können.}\\
...
\textbullet{} \emph{fA13 Der Anwender muss in der Lage sein Widgets nach seinen Wünschen über einen Drag and Drop Mechanismus auf dem Workspace zu sortieren .}\\
Befindet sich der Anwender auf einem Workspace und hat dieser Workspace mehr als ein Widget, so ist der Anwender in der Lage mit einem Klick auf die Widget-Statusleiste die Widgets per Drag and Drop auf dem Workspace neu anzuordnen (siehe Abbildung TODO). Die neuen Positionen werden direkt an den Server weitergegeben und somit persistiert.
